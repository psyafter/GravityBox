...
import com.ceco.v.gravitybox.managers.SysUiTunerManager;
...
public class SystemPropertyProvider {
    ...
    // Signature-level permission gating the handshake endpoint; only the same-signed
    // GravityBox app may send ACTION_GET_SYSTEM_PROPERTIES / ACTION_REGISTER_UUID.
    // Must match the <permission> declared in AndroidManifest.xml.
    public static final String PERMISSION_SYSTEM_PROPERTIES =
            "com.ceco.v.gravitybox.permission.SYSTEM_PROPERTIES";
        ...
        try {
            ...
            XposedHelpers.findAndHookMethod(classSystemUIService, "onCreate", new XC_MethodHook() {
                ...
                protected void afterHookedMethod(final MethodHookParam param) {
                    ...
                    final Context suiContext = (Context) param.thisObject;
                    final BroadcastMediator.Receiver handler = (context, intent) -> {
                    ...
                    // Register directly (not via the shared BroadcastMediator) so this
                    // sensitive endpoint is gated by a signature-level permission. The shared
                    // mediator must stay permission-free because it also carries protected
                    // system broadcasts that the OS would otherwise be unable to deliver.
                    final IntentFilter filter = new IntentFilter();
                    filter.addAction(ACTION_GET_SYSTEM_PROPERTIES);
                    filter.addAction(ACTION_REGISTER_UUID);
                    final BroadcastReceiver receiver = new BroadcastReceiver() {
                        @Override
                        public void onReceive(Context context, Intent intent) {
                            handler.onBroadcastReceived(context, intent);
                        }
                    };
                    try {
                        Utils.registerReceiver(suiContext, receiver, filter,
                                PERMISSION_SYSTEM_PROPERTIES, true);
                    } catch (Throwable t) {
                        GravityBox.log(TAG, "Error registering system properties receiver: ", t);
                    }
                }
            });
        } catch (Throwable t) {
